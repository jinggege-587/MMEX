...
;
MainArea.prototype.onTouchMove = function(a, c) {
    var b = ChartManager.getInstance();
    if (b._capturingMouseArea == this) {
        if (this._dragStarted == false) {
            if (Math.abs(this._oldX - a) >=0 || Math.abs(this._oldY - c) >=0) {
                this._dragStarted = true
            }
        }
    }
    ...
    if (this._passMoveEventToToolManager && b.onToolTouchMove(this.getFrameName(), a, c)) {
        b.hideCursor();
        return this
    }
    switch (b._drawingTool) {
...
;
ChartManager.prototype.onToolTouchMove = function(b, a, f) {
    var c = false;
    b += ".";
    for (var e in this._dataSources) {
        if (e.indexOf(b) == 0) {
...
ChartManager.prototype.onTouchMove = function(f, k, h, g) {
    ...
    if (this._capturingMouseArea != null ) {
        this._capturingMouseArea.onTouchMove(k, h);
        return
    }
    var b = c.contains(k, h);
    if (b == null ) {
...
DarkTheme.prototype.__construct = function() {
    this._colors = [];
	this._colors[Theme.Color.Positive] = "rgba(17,190,132,1)";
    this._colors[Theme.Color.Negative] = "rgba(253,55,5,1)";
	...
	this._colors[Theme.Color.TextPositive] = "rgba(17,190,132,0.50)";
    this._colors[Theme.Color.TextNegative] = "rgba(253,55,55,0.50)";
...
LightTheme.prototype.__construct = function() {
    ...
    this._colors[Theme.Color.Positive] = "rgba(17,190,132,1)";
    this._colors[Theme.Color.Negative] = "rgba(253,55,5,1)";
    this._colors[Theme.Color.PositiveDark] = "rgba(17,190,132,1)";
    this._colors[Theme.Color.NegativeDark] = "rgba(253,55,5,1)";
    this._colors[Theme.Color.Unchanged] = "#fff";
    this._colors[Theme.Color.Background] = "#fff";
    this._colors[Theme.Color.Cursor] = "#eeeeee";
    ...
    this._colors[Theme.Color.Grid4] = "#eeeeee";
    this._colors[Theme.Color.TextPositive] = "rgba(17,190,132,0.50)";
    this._colors[Theme.Color.TextNegative] = "rgba(253,55,55,0.50)";
    this._colors[Theme.Color.Text0] = "#4a5f78";
    this._colors[Theme.Color.Text1] = "#4a5f78";
    this._colors[Theme.Color.Text2] = "#4a5f78";
    this._colors[Theme.Color.Text3] = "#4a5f78";
    this._colors[Theme.Color.Text4] = "#4a5f78";
    this._colors[Theme.Color.LineColorNormal] = "#eeeeee";
    this._colors[Theme.Color.LineColorSelected] = "#393c40";
    this._colors[Theme.Color.CircleColorFill] = "#ffffff";
    this._colors[Theme.Color.CircleColorStroke] = "#eeeeee";
    this._fonts = [];
    this._fonts[Theme.Font.Default] = "12px Tahoma"
}
;
...
IndicatorPlotter.prototype.drawVolumeStick = function(b, s, m, f, j, o, p, r, k) {
    ...
    if (q.length > 0) {
        b.fillStyle = s.getColor(Theme.Color.TextPositive);
        b.strokeStyle = s.getColor(Theme.Color.TextPositive);
    ...
    if (g.length > 0) {
        b.fillStyle = s.getColor(Theme.Color.TextNegative);
...
function KLineMouseEvent() {
    $(document).ready(function() {
        ...
        $("#chart_toolbar_periods_hour ul a").click(function() {
            switch_period($(this).parent().attr("name"))
        });
		...
		$("#chart_overlayCanvas").on("touchstart", function(d){
			startX = d.originalEvent.targetTouches[0].clientX;
			startY = d.originalEvent.targetTouches[0].clientY;
		});
		$("#chart_overlayCanvas").on("touchmove", function(f){
			var c = f.target.getBoundingClientRect();
			var b = f.originalEvent.targetTouches[0].clientX- c.left;
			var g = f.originalEvent.targetTouches[0].clientY - c.top;
			var d = ChartManager.getInstance();
			ChartManager.getInstance().onMouseDown("frame0", b, g);
			GLOBAL_VAR.button_down=true;
			d.onTouchMove("frame0", b, g, true);
			d.redraw("All", false);
		});
...
function on_size() {
    ...
    var s = $("#chart_toolbar_periods_hour");
...
function switch_period(a) {
    ...
    $("#chart_toolbar_periods_hour ul a").removeClass("selected");
    ...
    $("#chart_toolbar_periods_hour ul a").each(function() {
        if ($(this).parent().attr("name") == a) {
            $(this).addClass("selected")
        }
    });
